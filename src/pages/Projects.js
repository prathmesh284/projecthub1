...
const filterOptions = {
  tech: ["All", "React", "Flutter", "Vue", "Next.js", "Flask", "Express", "Spring Boot", "Hibernate", "Node.js", "Python", "MongoDB"],
  type: ["All", "Frontend", "Backend", "Fullstack", "Data Science", "Machine Learning", "Web Application", "ML Tool"],
  domain: ["All", "AIML", "Cybersecurity", "Web Development", "App Development", "Healthcare", "Portfolio", "Event Management"],
  status: ["All", "Active", "Archived", "In Progress"]
};
...
const ProjectGrid = () => {
  ...
  useEffect(() => {
    axios.get("/api/projects")
      .then((res) => {
        const sorted = res.data.sort((a, b) => levelOrder[a.level] - levelOrder[b.level]);
        setProjects(sorted);
      })
  ...
  const filteredProjects = projects.filter((project) => {
    const techMatch = filters.tech === "All" || project.tech.includes(filters.tech);
    const typeMatch = filters.type === "All" || project.type === filters.type;
    const domainMatch = filters.domain === "All" || project.domain === filters.domain;
    const statusMatch = filters.status === "All" || project.status === filters.status;
    return techMatch && typeMatch && domainMatch && statusMatch;
  });

  return (
    <div className="p-6 scroll-auto overflow-y-auto h-[80vh] scrollbar-hide">
      <div className="max-w-6xl mx-auto mb-6 flex flex-wrap gap-8 justify-center">
            ...
            filteredProjects.map((project) => {
              ...
              return (
                ...
                >
                  ...
                  {/* 🔒 Lock overlay for Intermediate & Expert */}
                  {isLocked && (
                    <div className="absolute inset-0 bg-white/70 backdrop-blur-sm z-20 flex items-center justify-center rounded-2xl">
                      <div className="text-gray-700 text-sm font-semibold flex flex-col items-center">
                        <svg
